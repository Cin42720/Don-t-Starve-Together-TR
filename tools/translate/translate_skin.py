"""
DST Skin Ceviri - Gemini 2.5 Flash ile Kostum/Skin Stringleri
===============================================================
OpenRouter uzerinden Gemini 2.5 Flash kullanarak
8,400+ skin stringini Turkceye cevirir.

Kullanim:
    python tools/translate/translate_skin.py
    python tools/translate/translate_skin.py --api-key sk-or-...
    python tools/translate/translate_skin.py --batch-size 30
    python tools/translate/translate_skin.py --category SKIN_NAMES
...
Karakter kostum/skin isimlerini, aciklamalarini ve sozlerini ceviriyorsun.

KURALLAR:
1. Karakter isimleri (Wilson, Willow, Maxwell, Wendy, Wolfgang, WX-78, Wickerbottom, Woodie, Wes, Webber, Wigfrid, Winona, Warly, Wormwood, Wurt, Walter, Wanda, Abigail) CEVRILMEZ.
2. Skin/kostum temalari: survivor (hayatta kalan), triumphant (muzaffer), shadow (golge), lunar (ay), formal (resmi), victorian (Viktorya donemi), rose (gul), ice (buz), nature (doga), ancient (kadim), yule (noel), masquerade (maskeli balo), steampunk, detective (dedektif), bee (ari), ventriloquist (karincagoz), bishop (piskopos).
3. Kisa ve oz ceviriler yap.
4. \\n gibi ozel karakterleri aynen koru.
5. Bos string gelirse bos dondur.
6. Skin isimleri genellikle tema + karakter formundadir. Ornek: "Survivor Wilson" -> "Hayatta Kalan Wilson"
7. Skin aciklamalari genellikle kisa ve sloganimsi cumlelerdir.
8. Skin sozleri (QUOTES) karakter kisiligi ile uyumlu olmalidir.
9. Oyundaki karanlik/gotik atmosferi koru.
10. [KATEGORI] bilgisi ceviriye dahil edilmez, sadece baglam icin kullan.
...
Ornek: {"1": "Hayatta Kalan Wilson", "2": "Karanlik gunler icin hazir.", "3": "Muzaffer Willow"}"""
...
def main():
    parser = argparse.ArgumentParser(description="DST Skin Ceviri")
    parser.add_argument("--api-key", default="")
    parser.add_argument("--batch-size", type=int, default=BATCH_SIZE)
    parser.add_argument("--category", default="", help="Sadece belirli kategori (SKIN_NAMES, SKIN_QUOTES, SKIN_DESCRIPTIONS)")
    parser.add_argument("--fresh", action="store_true", help="Mevcut cevirileri sil, sifirdan basla")
    args = parser.parse_args()

    print()
    print("=" * 60)
    print("  DST SKIN CEVIRI - Gemini 2.5 Flash")
    print("  Kostum/skin isimleri ve aciklamalari cevirisi")
    print("=" * 60)
    ...
    en_path = os.path.join(JSON_DIR, "skin_en.json")
    tr_path = os.path.join(JSON_DIR, "skin_tr.json")

    if not os.path.isfile(en_path):
        print(f"\n  [HATA] {en_path} bulunamadi.")
        print("  Once tools/extract/extract_skin.py calistirin.")
        sys.exit(1)
    ...
    to_translate = {}
    for key, value in skin_en.items():
        if not value or not value.strip():
            continue
        if args.category and not key.startswith(f"{args.category}."):
            continue
        if key not in skin_tr or not skin_tr[key]:
            to_translate[key] = value

    total_en = len(skin_en)
    already_done = sum(1 for k in skin_en if k in skin_tr and skin_tr[k])
    remaining = len(to_translate)

    print(f"\n  Toplam skin string:  {total_en:,}")
    print(f"  Zaten cevrilmis:     {already_done:,}")
    print(f"  Cevrilecek:          {remaining:,}")

    if remaining == 0:
        print("\n  Tum skin stringler cevrilmis!")
        return

    avg_chars = sum(len(v) for v in to_translate.values()) / max(remaining, 1)
    est_input_tokens = remaining * (avg_chars / 3.5 + 15)
    ...
    try:
        for i in range(0, len(keys_list), batch_size):
            ...
            prompt = f"""Asagidaki {len(batch_keys)} Don't Starve Together skin/kostum stringini Turkceye cevir.
Koseli parantez icindeki kisim KATEGORI bilgisidir:
- SKIN_NAMES = Kostum isimleri (kisa, 2-4 kelime)
- SKIN_DESCRIPTIONS = Kostum aciklamalari (kisa cumleler)
- SKIN_QUOTES = Karakter sozleri (kostumle ilgili kisa replikler)
...
SADECE JSON dondur: {{"1": "ceviri", "2": "ceviri", ...}}"""
            ...
            if batch_num % SAVE_EVERY == 0:
                with open(tr_path, "w", encoding="utf-8") as f:
                    json.dump(skin_tr, f, ensure_ascii=False, indent=2, sort_keys=True)
                done_pct = len(skin_tr) * 100 / total_en
                print(f"  >>> Kaydedildi: {len(skin_tr):,}/{total_en:,} (%{done_pct:.1f})")
        ...
        with open(tr_path, "w", encoding="utf-8") as f:
            json.dump(skin_tr, f, ensure_ascii=False, indent=2, sort_keys=True)
        print(f"  Kaydedildi: {len(skin_tr):,} ceviri")
    ...
    elapsed = time.time() - start_time
    final_pct = len(skin_tr) * 100 / total_en

    print()
    print("=" * 60)
    print(f"  TAMAMLANDI!")
    print(f"  Yeni ceviri:    {translated_count:,}")
    print(f"  Toplam ceviri:  {len(skin_tr):,}/{total_en:,} (%{final_pct:.1f})")
